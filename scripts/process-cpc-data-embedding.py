...
parser.add_argument(
	...
	default='../data/raw/mm-cpc/train/data-*/*.csv.gz',
	help='Paths to training data. Supports globbing.')
parser.add_argument(
	'--validation_source', type=str,
	default=(
		'../data/raw/mm-cpc/validation/'
		'part-00000-f7421232-0a5f-43a1-a30d-359dd1e1b618-c000.csv.gz'),
	help='Path to validation data.')
parser.add_argument(
	'--test_source', type=str,
	default=(
		'../data/raw/mm-cpc/test/'
		'part-00000-66d695ad-776c-44b9-8feb-9f3906f297a0-c000.csv.gz'),
	help='Path to test data.')
parser.add_argument(
	'--destination_dir', type=str, default='../data/processed/mm-cpc/',
	help='Directory where processed CSVs are saved.')
parser.add_argument(
	'--intermediate_dir', type=str, default='../data/intermediate/mm-cpc/',
...
all_csv_paths = train_csv_paths + [TEST_SOURCE, VALIDATION_SOURCE]
print(f'Found {len(all_csv_paths)} CSV files.')

all_dfs = [pd.read_csv(path) for path in all_csv_paths]
all_df = pd.concat(all_dfs, axis='rows')
print(
...
drop_features = drop_features.union(set(['column_weights']))
