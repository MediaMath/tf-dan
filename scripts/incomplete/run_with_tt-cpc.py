...
import dan.utils as ut

from collections import OrderedDict
from dan import EmbeddingTransformer, Disguise, Discriminator, DAN
...
parser.opt_range(
	'--lambd', type=float, default=0.05, tunable=True, low=.01, high=10, log_base=2,  
	help='Amount of regularization to apply to disguise loss.')
parser.opt_range(
	'--eta', type=float, default=0.2, tunable=True, low=.05, high=2, log_base=2,
...
def train_main(args):
	...
	LAMBDA = args.lambd
	ETA = args.eta
	DISGUISE_ARCHITECTURE = [128, 128, 128, 128]
	DISCRMINATOR_ARCHITECTURE = [64, 32, 16]
	NO_DISGUISE = args.no_disguise
	TEST = args.test

	exp = Experiment(name='my_test', save_dir=args.experiment_dir, autosave=False)
	exp.argparse(args)
...
hparams.optimize_parallel_gpu(train_main, gpu_ids = ['0','1','2','3','4','5','6','7'], nb_trials = 100, nb_workers = 8)
